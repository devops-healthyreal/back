...
public class JwtUtils {
    ...
    public JwtUtils(@Value("${security.secret.key}") String secretKey) {
    ...
    public String create(String userId) {
        ...
        String jwt = Jwts.builder()
                ...
                .subject(userId)//사용자 ID 설정sub
                .issuedAt(new Date())//생성 시간을 설정
                .expiration(expireTime)//만료 시간 설정(필수로 설정하자.왜냐하면 토큰(문자열이라)은 세션처럼 제어가 안된다)
                .signWith(SecurityConstants.SECRET_KEY,Jwts.SIG.HS256)
                .compact();
    ...
    public String generateToken(Authentication authentication){
        ...
        return Jwts.builder()
                ...
                .issuedAt(new Date())//생성 시간을 설정
                .expiration(expireDate)//만료 시간 설정(필수로 설정하자.왜냐하면 토큰(문자열이라)은 세션처럼 제어가 안된다)
                .signWith(SecurityConstants.SECRET_KEY,Jwts.SIG.HS256)
                .compact();
    }
    ...
    public String validate(String jwt) {
        ...
        try {
            subject = Jwts.parser()
                    .verifyWith(SecurityConstants.SECRET_KEY).build()
    ...
    public long getRemainTime(String jwt) {
        Claims claim = Jwts.parser().verifyWith(SecurityConstants.SECRET_KEY).build()
